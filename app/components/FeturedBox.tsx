"use client";

import { motion } from "framer-motion";
import { useEffect, useRef, useState } from "react";
import axios from "axios";
import Image from "next/image";
import { useRouter } from "next/navigation";

interface BestSellerProduct {
  id: number;
  name: string;
  price: number;
  image_url: string[];
}

interface BestSellerBox {
  id: number;
  title: string;
  description: string;
  is_active: boolean;
  parts: BestSellerProduct[];
}

export default function FeatureSlider() {
  const router = useRouter();
  const [box, setBox] = useState<BestSellerBox | null>(null);

  const trackRef = useRef<HTMLDivElement>(null);

  /* ---------- دریافت باکس پرفروش ---------- */
  useEffect(() => {
    axios.get("/api/FeatureBox").then((res) => {
      const data: BestSellerBox[] = res.data || [];
      const activeBox = data.find(
        (item) => item.is_active && item.parts.length > 0
      );
      if (activeBox) setBox(activeBox);
    });
  }, []);

  if (!box) return null;

  const scroll = (dir: "left" | "right") => {
    if (!trackRef.current) return;
    trackRef.current.scrollBy({
      left: dir === "left" ? -300 : 300,
      behavior: "smooth",
    });
  };

  return (
    <div className="container mx-auto p-3">
      <h2 className="text-xl sm:text-2xl mb-3 sm:mb-6 text-[#008BDF] font-yekanExtraBold">
        {box.title}
      </h2>

      <section className="rounded-3xl p-3 bg-[#004D7A]">
        <div className="relative overflow-hidden px-3 sm:px-4">
          <motion.div
            ref={trackRef}
            className="flex gap-2 lg:gap-6 overflow-x-hidden cursor-grab hide-scrollbar"
            drag="x"
            dragConstraints={{ left: 0, right: 0 }}
            dragElastic={0.2}
            onDrag={(e, info) => {
              if (trackRef.current) {
                trackRef.current.scrollLeft -= info.delta.x;
              }
            }}
          >
            {box.parts.map((product) => (
              <div
                key={product.id}
                className="flex-shrink-0 w-[160px] sm:w-[190px] lg:w-[260px] bg-white rounded-3xl p-2 shadow-xl flex flex-col"
              >
                <div className="relative w-full h-40 sm:h-56">
                  <Image
                    src={product.image_url?.[0] || "/placeholder.png"}
                    alt={product.name}
                    fill
                    className="object-cover"
                  />
                </div>

                <div className="mt-4 text-black font-yekanDemiBold text-sm line-clamp-2">
                  {product.name}
                </div>

                <div className="flex flex-row gap-1 items-center mt-2">
                  <div className="text-base sm:text-[20px] text-[#004D7A] font-yekanDemiBold">
                    {product.price.toLocaleString()}
                  </div>
                  <span className="text-[12px] text-[#004D7A] font-yekanDemiBold">
                    تومان
                  </span>
                </div>

                <button
                  onClick={() => router.push(`/product/${product.id}`)}
                  className="mt-auto flex items-center text-xs sm:text-sm text-[#008BDF] font-yekanExtraBold"
                >
                  مشاهده جزئیات و خرید
                  <Image src="/Arrow-leftB.svg" alt="" width={16} height={16} />
                </button>
              </div>
            ))}
          </motion.div>

          {/* دکمه‌ها */}
          <button
            onClick={() => scroll("left")}
            className="absolute left-2 top-1/2 -translate-y-1/2 bg-white w-10 h-10 rounded-full shadow flex items-center justify-center"
          >
            <Image src="/Arrow-leftB.svg" alt="" width={16} height={16} />
          </button>

          <button
            onClick={() => scroll("right")}
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-white w-10 h-10 rounded-full shadow flex items-center justify-center"
          >
            <Image src="/Arrow-rightB.svg" alt="" width={16} height={16} />
          </button>
        </div>
      </section>
    </div>
  );
}
